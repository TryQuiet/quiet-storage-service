 ...
 * of their admission on the server that admitted them. The adversary is the joiner: a peer holding
 * a valid invitation is entitled to join but not to join unrecorded, and it is exactly the party
 * that cannot be relied on to report its own admission afterwards. localfirst/auth calls this on
 * the admitting side after it has appended an ADMIT_* link to the in-memory team and before it
 * queues the acceptance that hands the invitee the graph and the team keyring. It must resolve
 * only once both are on disk, and reject if either write fails, so that neither a crash nor a
 * joiner exploiting one can produce a member with keys but no record (QSS-006 / private#203,
 * threat-model C3 "Option A").
 */
